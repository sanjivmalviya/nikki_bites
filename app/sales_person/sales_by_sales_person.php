...
<?php
   ...
   if(isset($request->sales_person_id)){
          ...
          if(isset($year) && $year != "" && isset($month) && $month != ""){
             ...
             $target_assigned = "SELECT target_category_id,COALESCE(SUM(target_category_amount),0) as target_assigned,sales_person_id FROM tbl_target WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' AND sales_person_id = '$sales_person_id' GROUP BY target_category_id,sales_person_id";
             ...
             $target_assigned = "SELECT target_category_id,COALESCE(SUM(target_category_amount),0) as target_assigned,sales_person_id FROM tbl_target WHERE sales_person_id = '$sales_person_id' GROUP BY target_category_id,sales_person_id";
             $target_assigned = getRaw($target_assigned);
             ...
             if(isset($target_assigned)){
               foreach($target_assigned as $rs){
                   ...
                   $target_achieved = "SELECT det.order_detail_id,det.order_product_igst,det.order_product_sgst,det.order_product_cgst,det.order_product_rate FROM `tbl_order_detail` det INNER JOIN tbl_product pro ON det.order_product_id  = pro.product_id INNER JOIN tbl_category cat ON cat.category_id = pro.category_id INNER JOIN tbl_target tgt ON tgt.target_category_id = cat.category_id WHERE tgt.sales_person_id = '$sales_person_id' AND cat.category_id = '".$rs['target_category_id']."' ";
