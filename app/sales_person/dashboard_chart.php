...
<?php
   ...
   if(isset($request->sales_person_id)){
          ...
          $total_sales_by_months = "SELECT MONTH(created_at) as month,COALESCE(SUM(sales_person_order_amount),0) as total_sales FROM `tbl_sales_person_target_detail` WHERE sales_person_id = '$sales_person_id' GROUP BY MONTH(created_at)";
