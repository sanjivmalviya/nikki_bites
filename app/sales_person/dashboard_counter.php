...
<?php
   ...
   if(isset($request->sales_person_id)){
          ...
          // sales person's target by current month           
          $target_assigned = "SELECT COALESCE(SUM(target_category_amount),0) as target_assigned FROM tbl_target WHERE sales_person_id = '$sales_person_id' AND YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' ";
          ...
          $target_achieved = "SELECT COALESCE(SUM(sales_person_order_amount),0) as total_achieved FROM tbl_sales_person_target_detail WHERE YEAR(created_at) = '$year' AND MONTH(created_at) = '$month' AND sales_person_id = '$sales_person_id' ";
